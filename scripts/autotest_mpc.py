...
class autotest():
    ...
    def init_planner(self, no_TA):
        ...
        num_waypoint = int(config["num_waypoint"])
        ...
        for i in range(num_agent):
            ...
            carmsg.pose.position.x = min(x_max, max(x_min, float(start_pose[0]) + random.uniform(-randomness[0], randomness[0])))
            carmsg.pose.position.y = min(y_max, max(y_min, float(start_pose[1]) + random.uniform(-randomness[1], randomness[1])))
        ...
        if no_TA:
            ...
            for i in range(num_agent - num_task % num_agent):
                for j in range(num_waypoint):
                    if len(goalmsg.goals[-i - 1].poses) > 0:
                        ...
                        if task == 1:
                            waypoint = config["car" + str(i + 1)]["waypoints"]
                        else:
                            waypoint = config["task"+ str(i + 1)]
                        #waypoint = config["car" + str(num_agent - i)]["start"]
                        print('here')
            ...
            for i in range(num_task):
                ...
                #waypoints = config["car" + str(i + 1)]['waypoints']
                #waypoints = config["task"+ str(i + 1)]
                for j in range(int(num_waypoint)):
                    goal = Pose()
                    goal.position.x = waypoints[j][0]
                    goal.position.y = waypoints[j][1]
                    goal.position.z = 0.0
        ...
        #print('goal msg.goals'+ str(goalmsg.goals[0].poses))
        #print('goalmsg '+ str(goalmsg))
        ...
        for test_sys in self.sys_type:
            ...
            if(test_sys == "TA+CLCBS+MPC" or test_sys == "TA+NHTTC"):
                ...
                if(self.num_agent > 3):
                    ...
                    print('num_agent ' + str(self.num_agent))
            ...
            print('shutting down clcbs and TA')
...
for i in range(1):
    #test = autotest("/home/arianaverges/catkin_ws/src/PuSHR-Noetic-main/mushr_pixelart_mpc/config/autotest.yaml", "ex" + str(i + 1))
    test = autotest("/home/arianaverges/catkin_ws/src/PuSHR-Noetic-main/mushr_pixelart_mpc/config/autotest.yaml", "ex2")
    test.run_autotest() 
